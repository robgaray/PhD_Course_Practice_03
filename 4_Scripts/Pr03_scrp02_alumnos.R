...
##################
### Graficar (verificar que se han cargado bien los archivos)
##################
...
### Referencia
##################
# lm() calculo de coeficientes de regresión lineal
# predict() usar un modelo sobre variables de entrada para obtener un resultado
...
# Grafica Q-Q
par(mfrow=c(2,1))
plot(Summary_month$Load.kWh, RL_I[[             ]], xlim=lim_Load, ylim=lim_Load)
abline(a=0, b=1)
...
# residuales
plot (Summary_month[,c(1,3)], type="l", ylim=lim_Load)
points(Summary_month$Month, RL_I[[             ]], type="l", col="red")
points(Summary_month$Month, RL_I[[             ]], type="l", col="blue")
par(mfrow=c(1,1))
...
Load_month_2020<-as.data.frame(prediccion)
...
lim_HDD <- c(0, max(Summary_month_2020$HDD))
lim_Load <- c(0, max(Summary_month_2020$Load.kWh))

par(mfrow=c(2,1))
plot(Summary_month$HDD, Summary_month$Load.kWh, xlim=lim_HDD, ylim=lim_Load)
abline(RL_I$coefficients[[1]], RL_I$coefficients[[2]])
points(Summary_month_2020$HDD, Summary_month_2020$Load.kWh, col="red")

plot (Summary_month_2020[,c(1,3)], type="l", ylim=lim_Load)
points(Summary_month_2020$Month, Summary_month_2020$Load.kWh, type="l", col="red")
par(mfrow=c(1,1))